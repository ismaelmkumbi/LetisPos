# POS True Split Panel — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a true split-panel POS layout where the cart is always visible on desktop and always accessible via a persistent bottom bar on mobile.

**Architecture:** A single new layout component (`SplitLayout.tsx`) that renders products and cart side-by-side on desktop, and products with a persistent bottom cart bar + bottom-sheet cart on mobile. Reuses all existing business logic from `PosTerminalPage.tsx` via the unchanged `PosLayoutProps` interface.

**Tech Stack:** React 19 + TypeScript, MUI 7, framer-motion, existing POS components (EditLineModal, CashRegisterIndicator, TotalRow)
...
```
Create: frontend/src/components/smartpos/PosLayouts/SplitLayout.tsx  — new layout component
Modify: frontend/src/views/smartpos/pos/PosTerminalPage.tsx          — register 'split' as default layout
```

---

### Task 1: Create SplitLayout Component

**Files:**
- Create: `frontend/src/components/smartpos/PosLayouts/SplitLayout.tsx`

- [ ] **Step 1: Write the SplitLayout component**
 ...
 * Mobile: products (full width) + fixed bottom bar with cart summary → cart opens as bottom sheet.
 */
...
import {
  IconBarcode, IconCheck, IconFilter, IconMinus, IconPlus,
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      >
        ...
        <Stack direction="row" spacing={0.5}>
          ...
          {isMobile && (
            <IconButton onClick={() => setCartOpen(false)} size="small"><IconX size={18} /></IconButton>
          )}
        </Stack>
      </Stack>
    ...
    return (
      ...
      >
        <Box sx={{ aspectRatio: { xs: '1/1', sm: '4/3' }, bgcolor: outOfStock ? brand.neutral[100] : brand.primary[50], display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          <Typography sx={{ fontSize: { xs: '1.4rem', sm: '1.8rem' }, fontWeight: 800, color: outOfStock ? brand.neutral[300] : brand.primary[200] }}>
            {p.name.charAt(0).toUpperCase()}
          </Typography>
          {stock && (
            <Chip size="small"
              label={outOfStock ? 'Out of stock' : lowStock ? `${stock.available} left` : `${stock.available}`}
              sx={{ position: 'absolute', top: 6, left: 6, height: 20, fontSize: '0.625rem', fontWeight: 800,
    ...
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
      ...
      <Box sx={{ px: 1.5, py: 1, bgcolor: '#fff', borderBottom: `1px solid ${brand.neutral[200]}`, display: 'flex', gap: 1, alignItems: 'center', flexShrink: 0, overflowX: 'auto', WebkitOverflowScrolling: 'touch', '&::-webkit-scrollbar': { display: 'none' }, scrollbarWidth: 'none' }}>
        ...
        <TextField select size="small" value={props.warehouseId}
          onChange={(e) => props.onWarehouseChange(e.target.value)} sx={selectFieldSx}>
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', bgcolor: '#F7F8FA', overflow: 'hidden' }}>
      ...
      {!isMobile ? (
        ...
        /* ── Mobile: full-width products + bottom bar ── */
        <>
          {productsPanel}
          ...
          {!cartOpen && (
            ...
            >
              ...
              <Chip
                label={itemCount > 0 ? 'View cart' : 'Open'}
                size="small"
...
```bash
git add frontend/src/components/smartpos/PosLayouts/SplitLayout.tsx
git commit -m "feat: add SplitLayout — true split-panel POS with always-visible cart
...
### Task 2: Register SplitLayout as Default in PosTerminalPage

**Files:**
- Modify: `frontend/src/views/smartpos/pos/PosTerminalPage.tsx`

- [ ] **Step 1: Add SplitLayout import and register as default**

Add the import (after the existing layout imports around line 51):

```typescript
import SplitLayout from 'src/components/smartpos/PosLayouts/SplitLayout';
```

Update the layout map (around line 599-605) to include 'split' and make it the default:
...
Then update the PosLayoutContext to default to 'split'. Read the context file first:

```bash
grep -n "layout\|setLayout\|DEFAULT" frontend/src/context/smartpos/PosLayoutContext.tsx
```

Find the default layout value (likely 'compact' or 'modern') and change it to 'split'.
...
```bash
git add frontend/src/views/smartpos/pos/PosTerminalPage.tsx frontend/src/context/smartpos/PosLayoutContext.tsx
git commit -m "feat: set SplitLayout as default POS layout
...
Open `http://localhost:5173/smartpos/pos` and verify:
- Desktop: products on left, cart always visible on right
- Mobile: products full width, persistent dark bottom bar showing cart total
- Tapping the bottom bar opens the cart as a bottom sheet
- Adding products updates the cart in real time
- Pay button works
- Existing layout switcher still works
