...
export async function listDebtors(params?: {
  ...
  // NOTE: This function makes per-customer API calls for enrichment. For shops
  // with < 50 credit customers this is fast enough. At larger scale, a dedicated
  // backend aggregation endpoint should replace this client-side grouping.
  ...
  if (salesPage.totalPages > 1 || partialPage.totalPages > 1) {
    console.warn(
      'listDebtors: not all unpaid sales fit in 500 results; some debt may be missing. Consider pagination.',
    );
  }
  ...
  for (const [customerId, sales] of byCustomer) {
    ...
    const oldest = sales.reduce((min, s) =>
      s.date < min ? s.date : min, sales[0].date);

    const now = Date.now();
    const overdue = new Date(oldest).getTime() < now - THIRTY_DAYS_MS;
...
export async function getCustomerDebt(customerId: UUID): Promise<{
  ...
  try {
    ...
    throw new Error(`Failed to fetch customer ${customerId}: ${err instanceof Error ? err.message : String(err)}`);
  }
  ...
  for (const sale of unpaidSales) {
    const age = now - new Date(sale.date).getTime();
...
export async function getTotalOutstanding(): Promise<{
  ...
  for (const sale of allUnpaid) {
    const due = sale.dueTotal || sale.grandTotal;
    total += due;
    if (new Date(sale.date).getTime() < thirtyDays) {
      overdueTotal += due;
    }
  }
