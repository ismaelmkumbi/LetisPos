...
import {
  createProduct, listProducts, deleteProduct, updateProduct,
...
export default function ProductsListPage() {
    ...
    const timer = setTimeout(() => {
      setSearchParams((prev) => {
        ...
        const set = (k: string, v: string | undefined) => {
          if (v && v !== '' && v !== 'all') next.set(k, v); else next.delete(k);
        };
  ...
  }, [search, statusFilter, categoryFilter, brandFilter, warehouseFilter, page, sort, setSearchParams]);
  ...
  const duplicateProduct = useCallback(async (p: Product) => {
    try {
      await createProduct({
        code: p.code ? `${p.code}-copy-${Date.now()}` : `${p.name.replace(/\s+/g, '-').toLowerCase()}-${Date.now()}`,
        name: `${p.name} (Copy)`,
        ...
        barcodes: p.barcodes?.map((b) => ({ barcode: b.barcode, barcodeType: b.barcodeType, primary: b.primary })),
        variants: p.variants?.map((v) => ({
          name: v.name, code: v.code ?? undefined, price: v.price ?? undefined,
          cost: v.cost ?? undefined, wholesalePrice: v.wholesalePrice ?? undefined,
          minPrice: v.minPrice ?? undefined, imageUrl: v.imageUrl ?? undefined,
        })),
  ...
  const handleMouseEnter = useCallback((p: Product, el: HTMLElement) => {
    hoverTimer.current = setTimeout(() => { setHoverProduct(p); setAnchorEl(el); }, 400);
  }, []);
  ...
  useEffect(() => {
    ...
    listWarehouses().then((rows) => setWarehouses(rows.filter((r) => r.active))).catch(() => {});
  }, []);

  // ── Fetch stock levels when warehouse filter is active ──────────────────────
  useEffect(() => {
    if (!warehouseFilter) { setStockLevelMap(new Map()); return; }
    let cancelled = false;
    listStockLevels(warehouseFilter, 0, 1000)
      ...
      .catch(() => { if (!cancelled) setStockLevelMap(new Map()); });
  ...
  // ── Lookup helpers ─────────────────────────────────────────────────────────
  const catName  = (id: string | null | undefined) => id ? (categories.find((c) => c.id === id)?.name ?? null) : null;
  const brandName = (id: string | null | undefined) => id ? (brands.find((b) => b.id === id)?.name ?? null) : null;
  ...
  const columns: Column<Product>[] = useMemo(() => [
    sel.selectionColumn(),
    {
      ...
      render: (p) => (
        <Tooltip title={p.name} placement="bottom-start">
          <Stack
            direction="row" spacing={1.25} alignItems="center" sx={{ minWidth: 0 }}
    ...
    },
    {
      ...
      render: (p) => (
        <Stack spacing={0.5}>
          ...
          <Typography sx={{ fontSize: '0.69rem', color: brand.neutral[500], letterSpacing: '0.06em' }}>
            ||||| |||| |||||
          </Typography>
        </Stack>
      ),
    },
    {
      ...
      render: (p) => {
        ...
        return name
          ? <Tooltip title={name}><Typography component="span" sx={{ color: brand.neutral[700], fontSize: '0.75rem' }} noWrap>{name}</Typography></Tooltip>
          : <Typography component="span" sx={{ color: brand.neutral[400], fontSize: '0.75rem' }}>—</Typography>;
      },
    },
    {
      key: 'brandId',
    ...
    {
      key: 'price', label: 'Retail Price', align: 'right', width: 138,
      sortable: true,
      exportValue: (p) => p.price,
      render: (p) => (
        <Typography component="span" sx={{ fontWeight: 700, color: brand.primary[700], fontVariantNumeric: 'tabular-nums', fontSize: '0.8125rem' }}>
          {fmt(p.price)}
        </Typography>
      ),
    },
    {
      key: 'wholesalePrice', label: 'Wholesale', align: 'right', width: 120,
      sortable: true,
      exportValue: (p) => p.wholesalePrice ?? '',
      render: (p) => p.wholesalePrice != null
        ? <Typography component="span" sx={{ color: brand.info.dark, fontWeight: 600, fontVariantNumeric: 'tabular-nums', fontSize: '0.75rem' }}>{fmt(p.wholesalePrice)}</Typography>
    ...
    },
    {
      ...
      render: (p) => (
        <Tooltip title="Minimum stock before alert fires">
          <Typography component="span" sx={{ fontSize: '0.82rem', fontWeight: 700, color: brand.neutral[700] }}>
            {p.stockAlert} pcs
          </Typography>
        </Tooltip>
      ),
    ...
    {
      key: 'stockHealth', label: 'Stock', align: 'center', width: 80,
      sortable: false,
      exportValue: (p) => {
      ...
      render: (p) => {
        const sl = stockLevelMap.get(p.id);
        if (!sl) return <Typography component="span" sx={{ color: brand.neutral[300], fontSize: '0.75rem' }}>—</Typography>;
        const ratio = sl.stockAlertThreshold > 0 ? sl.onHand / sl.stockAlertThreshold : 2;
        const pct = Math.min(ratio / 2, 1) * 100;
        const barColor = ratio >= 1.5 ? brand.success.dark
          : ratio >= 1 ? brand.warning.dark
          : ratio >= 0.5 ? brand.warning.main
          : brand.error.dark;
        const barBg = ratio >= 1.5 ? brand.success.light
          : ratio >= 1 ? brand.warning.light
          : '#FFF3E0';
        return (
          <Tooltip title={`${sl.onHand} on hand · ${sl.available} available · Alert at ${sl.stockAlertThreshold}`}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.75 }}>
              <Box sx={{ width: 36, height: 6, bgcolor: brand.neutral[100], borderRadius: '3px', overflow: 'hidden', flexShrink: 0 }}>
                <Box sx={{ width: `${pct}%`, height: '100%', bgcolor: barColor, borderRadius: '3px', transition: 'width 0.3s ease' }} />
              </Box>
              <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: barColor, bgcolor: barBg, px: 0.5, py: 0.1, borderRadius: '4px', lineHeight: 1.3, whiteSpace: 'nowrap' }}>
                {sl.onHand}
              </Typography>
            </Box>
          </Tooltip>
    ...
    {
      key: 'dataQuality', label: 'Quality', align: 'center', width: 72,
      sortable: true,
      exportValue: (p) => `${dataQualityScore(p)}%`,
      render: (p) => {
        const score = dataQualityScore(p);
        const color = score >= 88 ? brand.success.dark
          : score >= 66 ? brand.warning.dark
          : score >= 33 ? brand.warning.main
          : brand.error.dark;
        const bg = score >= 88 ? brand.success.light
          : score >= 66 ? brand.warning.light
          : score >= 33 ? '#FFF3E0'
          : brand.error.light;
        return (
          <Tooltip title={`Data quality: ${score}%`}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
              ...
              <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color, bgcolor: bg, px: 0.6, py: 0.1, borderRadius: '4px', lineHeight: 1.4 }}>
                {score}%
              </Typography>
            </Box>
          </Tooltip>
    ...
    },
    {
      ...
      render: (p) => <StatusBadge label={p.status ? 'Active' : 'Inactive'} tone={p.status ? 'success' : 'neutral'} />,
    },
    {
      key: 'actions',
      label: '',
      ...
      enableHiding: false,
      render: (p) => (
        <Stack direction="row" justifyContent="flex-end" spacing={0.3} onClick={(e) => e.stopPropagation()}>
          <Tooltip title="Duplicate product">
            <IconButton
              size="small"
              onClick={(e) => { e.stopPropagation(); duplicateProduct(p); }}
              sx={actionBtnSx}
            >
              <IconCopy size={13} />
            </IconButton>
          ...
          <IconButton
            size="small"
            onClick={(e) => { e.stopPropagation(); navigate(`/smartpos/products/${p.id}/edit`); }}
            sx={actionBtnSx}
          >
  ...
  if (statusFilter !== 'all') activeFilters.push({ key: 'status', label: `Status: ${statusFilter === 'active' ? 'Active' : 'Inactive'}`, clear: () => { setStatusFilter('all'); setPage(0); } });
  if (categoryFilter) activeFilters.push({ key: 'cat', label: `Category: ${catName(categoryFilter) ?? '…'}`, clear: () => { setCategoryFilter(''); setPage(0); } });
  if (brandFilter)    activeFilters.push({ key: 'brand', label: `Brand: ${brandName(brandFilter) ?? '…'}`,   clear: () => { setBrandFilter('');   setPage(0); } });
  if (warehouseFilter) activeFilters.push({ key: 'wh', label: `Warehouse: ${warehouseName(warehouseFilter)}`, clear: () => { setWarehouseFilter(''); setPage(0); } });
    ...
    <Box>
        ...
        renderExpanded={(p) => (
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2.5} alignItems="flex-start">
            <Avatar
              src={p.imageUrl ?? undefined}
              variant="rounded"
              sx={{ width: 80, height: 80, borderRadius: '10px', flexShrink: 0, bgcolor: brand.primary[50], color: brand.primary[700], fontWeight: 700, fontSize: 24 }}
            >
              {p.name.charAt(0)}
            </Avatar>
            <Box sx={{ flex: 1, width: '100%' }}>
              ...
              {p.description && (
                <Typography sx={{ fontSize: '0.8rem', color: brand.neutral[500], mb: 1.5, lineHeight: 1.5 }}>
                  {p.description}
                </Typography>
              )}
              <Stack
                ...
                useFlexGap
                flexWrap="wrap"
                sx={{ '& > *': { flex: '0 0 33.33%', py: 0.75, borderBottom: `1px solid ${brand.neutral[100]}` } }}
              >
                {[
                  ['SKU', p.code],
                  ...
                  ['Stock on Hand', (() => { const sl = stockLevelMap.get(p.id); return sl ? `${sl.onHand} pcs (${sl.available} available)` : '—'; })()],
                  ['Reorder Level', p.stockAlert != null ? `${p.stockAlert} pcs` : '—'],
                  ['Status', p.status ? 'Active' : 'Inactive'],
                ].map(([label, value]) => (
                  <Stack key={label} direction="row" spacing={1} alignItems="center" sx={{ px: 1.5 }}>
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, color: brand.neutral[500], minWidth: 90 }}>{label}</Typography>
                    <Typography sx={{ fontSize: '0.8rem', fontWeight: 700, color: brand.neutral[800] }}>{value}</Typography>
                  </Stack>
                ))}
              </Stack>
            </Box>
            <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ flexShrink: 0 }}>
              ...
              {p.trackSerial && <Chip label="Serialized" size="small" sx={{ height: 22, fontWeight: 700, fontSize: '0.65rem', bgcolor: brand.warning.light, color: brand.warning.dark, borderRadius: '6px' }} />}
              {p.trackImei && <Chip label="IMEI" size="small" sx={{ height: 22, fontWeight: 700, fontSize: '0.65rem', bgcolor: brand.info.light, color: brand.info.dark, borderRadius: '6px' }} />}
            </Stack>
          </Stack>
        )}
      />
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField
              ...
              label={batchPriceMode === 'fixed' ? 'New price' : 'Percentage change (e.g. 10 or -10)'}
      ...
      <Dialog open={batchStockOpen} onClose={() => !batchStockProcessing && (setBatchStockOpen(false), setBatchStockAlertValue(''))} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            fullWidth size="small" type="number" label="Stock alert quantity"
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setBatchStockOpen(false)} disabled={batchStockProcessing}>Cancel</Button>
          <Button variant="contained" color="primary" onClick={handleBatchStock} disabled={!batchStockAlertValue || batchStockProcessing}>
      ...
      {hoverProduct && anchorEl && (
        ...
        >
          <Paper sx={{ p: 1.5, minWidth: 240, maxWidth: 300, boxShadow: '0 8px 24px rgba(15,23,42,0.12)' }}>
            <Stack spacing={1}>
              <Stack direction="row" spacing={1} alignItems="center">
                <Avatar
                  src={hoverProduct.imageUrl ?? undefined} variant="rounded"
                  sx={{ width: 40, height: 40, fontSize: 14, fontWeight: 700, bgcolor: brand.primary[50], color: brand.primary[700], borderRadius: '8px', border: `1px solid ${brand.neutral[200]}` }}
                >
                  {hoverProduct.name.charAt(0)}
                </Avatar>
                <Box sx={{ minWidth: 0 }}>
                  <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '0.8125rem' }} noWrap>{hoverProduct.name}</Typography>
                  <Typography variant="caption" sx={{ fontFamily: 'ui-monospace', fontWeight: 600, color: brand.neutral[500] }} noWrap>{hoverProduct.code}</Typography>
                </Box>
              </Stack>
              <Stack spacing={0.5}>
                {[
                  ...
                  ...(hoverProduct.wholesalePrice != null ? [['Wholesale', fmt(hoverProduct.wholesalePrice), brand.info.dark]] : []),
              ...
              <Stack direction="row" spacing={0.5} useFlexGap flexWrap="wrap">
                {hoverProduct.featured    && <Chip label="Featured"     size="small" sx={{ height: 18, fontSize: '0.625rem', fontWeight: 700, bgcolor: brand.accent[50],   color: brand.accent[700],   borderRadius: '4px' }} />}
                {hoverProduct.sellable === false && <Chip label="Hidden" size="small" sx={{ height: 18, fontSize: '0.625rem', fontWeight: 700, bgcolor: brand.neutral[100], color: brand.neutral[600], borderRadius: '4px' }} />}
                {hoverProduct.variant     && <Chip label="Has variants" size="small" sx={{ height: 18, fontSize: '0.625rem', fontWeight: 700, bgcolor: brand.info.light,   color: brand.info.dark,   borderRadius: '4px' }} />}
                {hoverProduct.trackSerial && <Chip label="Serialized"   size="small" icon={<IconAlertTriangle size={10} />} sx={{ height: 18, fontSize: '0.625rem', fontWeight: 700, bgcolor: brand.warning.light, color: brand.warning.dark, borderRadius: '4px' }} />}
              </Stack>
            </Stack>
          </Paper>
        </Box>
